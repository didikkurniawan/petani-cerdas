...
function onClickDetailKalibrasi(param) {
    ...
    for (var i = 0; i < order.qty; i++) {
        var _row = "<tr>" +
            ...
            "<td><select class='form-control select_tindakan input-sm'><option value='selesai'>Selesai</option><option value='dibatalkan'>Dibatalkan</option></select></td>" +
            "<td><input type='file' class='form-control input-sm file_pengolahan_data' /></td>" +
            "</tr>";
    ...
    $('#form_detail_kalibrasi').submit(function (e) {
        ...
        Promise.all(tasks).then(function (results) {
            ...
            $.ajax({
                ...
                toastr.success('Berhasil Update Pengolahan Data', 'Berhasil!');
                $('#modal_detail_kalibrasi').modal('hide');
            }, function (err) {
                toastr.error(err.toString(), 'Terjadi Kesalahan Sistem!');
            });
